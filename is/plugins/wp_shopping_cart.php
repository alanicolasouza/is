...
} else if ($_POST['cquantity'] && !empty(  $_SESSION['pssimpleCart']  ) ) {
    ...
    if ($_SESSION['pssimpleCart'] && is_array($_SESSION['pssimpleCart']))
    {
	    ...
	    {    	
	        ...
	        <tr>
	            ...
	            <td class='padding10' style='text-align: center; border-right: 1px solid #CCCCCC; border-bottom: 1px solid #CCCCCC;'>
	                ...
	                <form method=\"post\"  action=\"\" name='pcquantity' style='display: inline'>
	                    ...
	                    <input type='number' name='quantity' value='".$item['quantity']."' size='2' class=\"change_quantity\" disabled />
	                </form>
	                
	                <form method=\"post\"  action=\"\">
	                    <input type='hidden' name='product' value='".$item['name']."' />
	                    <input type='hidden' name='delcart' value='1' />
	                    <input type='submit' value='&nbsp;' title='Remover' />
	                </form>
	            </td>
	
	            <td class='padding10' style='text-align: center;  border-bottom: 1px solid #CCCCCC;'>".money( ( str_replace(',', '.', $item['price'] ) * $item['quantity'] ), 'br')."</td>
	        </tr>";
	        ...
	        $count++;
       		...
       		<tr>
                <td style=' position: absolute;' colspan='4'>";


              	$output .= '<form id="pagamento" method="post" action="https://pagseguro.uol.com.br/v2/checkout/payment.html">' . $form;
            		...
            		$output .= '
							    ...
							    <div class="alert alert-info">
							    	<p>
					   	 	 			Preencha o campo <strong>CEP</strong>  abaixo, este campo é obrigatório para o envio do(s) produto(s).
					   	 	 		</p>
					   	 	 		
						   	 	 	<input type="text" maxlength="8" id="cep" placeholder="CEP"  />
						   	 	 	<input type="hidden" name="peso_total"  value="'.str_replace('.', ',', $peso).'" id="descricao_frete" />
						   	 	 	<input type="hidden" name="id_frete" 	value="'.$count.'" id="id_frete" />  
	
						   	 	 	<div id="frete" class="clearfix"> </div>

 ...
 * @since 0.1.1
 */
class Wordwebpress_Stylesheet 
{
	
	/**
	 * Cont�m os nome e conteudo dos arquivos
	 * a serem renderizados na p�gina
	 *
	 * @var array
	 */
	...
	/**
	 * Adiciona o css a cole��o para posteriormente no theme
	 * ser rederizado como tal, seguindo a conven��o que todos
	 * os stylesheets estar�o na pasta %theme%/%assets_path%/css
	 * caso n�o exista o stylesheet ele retorna um coment�rio
	 * de erro ao html
	 *
	 * @param string $name
	 *       	 Nome do arquivo css
	 * @param string $rack
	 *       	 Express�o para rack de css em navegadores
	 * @param string $vendor
	 *       	 Verdadeiro para bibliotecas de terceiros
	 * @since 1.3
	 * @return Wordwebpress_Stylesheet
	 */
	public function include_script_css_file($name, $rack = null, $vendor = false) {
		if (file_exists ( WORDWEBPRESS_INCLUDE . Wordwebpress::getInstance()->configuration()->get_cfn_value ( 'assets_path' ) . '/css/' . $name . '.css' )) {
			if ($rack) :
				$this->_css ['rack'] [$rack] [$name] = $name;
				Wordwebpress::getInstance()->debug()->_debug_add_content ( "File: {$name}.css add on css container (rack)" );
			elseif ( $vendor ) :
				$this->_css ['vendor'] [$name] = $name;
				Wordwebpress::getInstance()->debug()->_debug_add_content ( "File: {$name}.css add on css container ( vendor )" );
			else :
				$this->_css ['archy'] [$name] = $name;
				Wordwebpress::getInstance()->debug()->_debug_add_content ( "File: {$name}.css add on css container" );
			endif;
			return $this;
		}
		$this->_css ['error'] [$name] = $name;
		Wordwebpress::getInstance()->debug()->_debug_add_content ( "File: {$name}.css not existent", true );
		return $this;
	}
	/**
	 * Adiciona o less a cole��o para posteriormente no theme
	 * ser rederizado como tal, seguindo a conven��o que todos
	 * os stylesheets less estar�o na pasta %theme%/%assets_path%/css
	 * caso n�o exista o stylesheet ele retorna um coment�rio
	 * de erro ao html
	 *
	 * @param string $name
	 ...
	 * @return Wordwebpress_Stylesheet
	 */
	public function include_script_less_file($name, $rack = null) {
		if (file_exists ( WORDWEBPRESS_INCLUDE . Wordwebpress::getInstance()->configuration()->get_cfn_value ( 'assets_path' ) . '/css/' . $name . '.less.css' )) {
			if (! $rack) :
				$this->_css ['less'] [$name] = $name;
				Wordwebpress::getInstance()->debug()->_debug_add_content ( "File: {$name}.less add on css container" );
			 else :
				$this->_css ['rackless'] [$rack] [$name] = $name;
				Wordwebpress::getInstance()->debug()->_debug_add_content ( "File: {$name}.less add on css container (rack)" );
			endif;
			return $this;
		}
		$this->_css ['error'] [$name] = $name;
		Wordwebpress::getInstance()->debug()->_debug_add_content ( "File: {$name}.less not existent", true );
		return $this;
	}
	/**
	 ...
	 * @return Wordwebpress_Stylesheet
	 */
	public function include_script_css($stylesheet) {
		$this->_css ['style'] [] = $stylesheet;
		return $this;
	}
	
	/**
	 * Imprime os arquivos e configura��es do css
	 * no tema
	 *
	 * @since 1.0
	 * @return Wordwebpress_Stylesheet
	...
	public function render_css() {
		if (sizeof ( $this->_css ['vendor'] )) {
			...
			foreach ( $this->_css ['vendor'] as $key => $item ) {
				Wordwebpress::getInstance()->debug()->_debug_add_content ( "File ( {$item} ).css initalizing successfully ( vendor )" );
				echo "<link type=\"text/css\" rel=\"stylesheet\" href=\"" . WORDWEBPRESS_URL . Wordwebpress::getInstance()->configuration()->get_cfn_value ( 'assets_path' ) . '/css/' . $item . '.css" />' . "\n";
			}
		}
		if (Wordwebpress::getInstance()->configuration()->get_cfn_value ( 'compress_css_js' )) {
			Wordwebpress::getInstance()->debug()->_debug_add_content ( "Starting the rendering of css" );
			$query = '';
			if (sizeof ( $this->_css ['archy'] )) {
				...
				if ($this->generate_css_min ( $query ))
					Wordwebpress::getInstance()->debug()->_debug_add_content ( "File ( {$query} ).css generated successfully" );
				else
					Wordwebpress::getInstance()->debug()->_debug_add_content ( "File ( {$query} ).css reloaded successfully" );
				echo "<link type=\"text/css\" rel=\"stylesheet\" href=\"" . WORDWEBPRESS_URL . Wordwebpress::getInstance()->configuration()->get_cfn_value ( 'assets_path' ) . Wordwebpress::getInstance()->configuration()->get_cfn_value ( 'cache_path' ) . '/_css_' . md5 ( $query ) . '.css" />' . "\n";
			}
			...
			if (sizeof ( $this->_css ['rack'] )) {
				foreach ( $this->_css ['rack'] as $key => $item ) {
					...
					if ($this->generate_css_min ( $query ))
						Wordwebpress::getInstance()->debug()->_debug_add_content ( "File ( {$query} ).css generated successfully (rack)" );
					else
						Wordwebpress::getInstance()->debug()->_debug_add_content ( "File ( {$query} ).css reloaded successfully (rack)" );
					echo "<!--[{$key}]><link type=\"text/css\" rel=\"stylesheet\" href=\"" . WORDWEBPRESS_URL . Wordwebpress::getInstance()->configuration()->get_cfn_value ( 'assets_path' ) . Wordwebpress::getInstance()->configuration()->get_cfn_value ( 'cache_path' ) . '/_css_' . md5 ( $query ) . '.css" /><![endif]-->' . "\n";
				}
			}
		} else {
			Wordwebpress::getInstance()->debug()->_debug_add_content ( "Not rendering of css" );
			
			if (sizeof ( $this->_css ['archy'] )) {
				sort ( $this->_css ['archy'] );
				foreach ( $this->_css ['archy'] as $key => $item ) {
					Wordwebpress::getInstance()->debug()->_debug_add_content ( "File ( {$item} ).css initalizing successfully" );
					echo "<link type=\"text/css\" rel=\"stylesheet\" href=\"" . WORDWEBPRESS_URL . Wordwebpress::getInstance()->configuration()->get_cfn_value ( 'assets_path' ) . '/css/' . $item . '.css" />' . "\n";
				}
			}
			...
			if (sizeof ( $this->_css ['rack'] )) {
				foreach ( $this->_css ['rack'] as $key => $item ) {
					foreach ( $item as $kkey => $iitem ) {
						Wordwebpress::getInstance()->debug()->_debug_add_content ( "File ( {$iitem} ).css initializing successfully (rack)" );
						echo "<!--[{$key}]><link type=\"text/css\" rel=\"stylesheet\" href=\"" . WORDWEBPRESS_URL . Wordwebpress::getInstance()->configuration()->get_cfn_value ( 'assets_path' ) . '/css/' . $iitem . '.css" /><![endif]-->' . "\n";
					}
				}
			}
		}
		if (sizeof ( $this->_css ['style'] )) {
			...
			foreach ( $this->_css ['style'] as $key => $item )
				echo $item;
			echo "\n</style>";
			Wordwebpress::getInstance()->debug()->_debug_add_content ( "Style added successfully on layout" );
		}
		if (sizeof ( $this->_css ['error'] )) {
			foreach ( $this->_css ['error'] as $key => $item )
				echo "\n<!-- scrips_css: " . WORDWEBPRESS_URL . Wordwebpress::getInstance()->configuration()->get_cfn_value ( 'assets_path' ) . '/css/' . $item . '.css INEXISTENTE -->';
		}
		return $this;
	}
	
	/**
	 * Imprime os arquivos e configura��es do less
	 * no tema
	 *
	 * @return Wordwebpress_Stylesheet
	 */
	public function render_less() {
		Wordwebpress::getInstance()->debug()->_debug_add_content ( "Starting the rendering of less" );
		$query = '';
		if (sizeof ( $this->_css ['less'] )) {
			$i = 0;
			sort ( $this->_css ['less'] );
			foreach ( $this->_css ['less'] as $key => $item ) {
				$query .= ($i > 0 ? ',' : '') . $item;
				$i ++;
			}
			if ($this->generate_less_min ( $query ))
				Wordwebpress::getInstance()->debug()->_debug_add_content ( "File ( {$query} ).less generated successfully" );
			else
				Wordwebpress::getInstance()->debug()->_debug_add_content ( "File ( {$query} ).less reloaded successfully" );
			echo "<link type=\"text/css\" rel=\"stylesheet\" href=\"" . WORDWEBPRESS_URL . Wordwebpress::getInstance()->configuration()->get_cfn_value ( 'assets_path' ) . Wordwebpress::getInstance()->configuration()->get_cfn_value ( 'cache_path' ) . '/_less_' . md5 ( $query ) . '.css" />' . "\n";
		}
		if (sizeof ( $this->_css ['rackless'] )) {
			foreach ( $this->_css ['rackless'] as $key => $item ) {
				$query = '';
				$i = 0;
				foreach ( $item as $kkey => $iitem ) {
					$query .= ($i > 0 ? ',' : '') . $iitem;
				...
				if ($this->generate_css_min ( $query ))
					Wordwebpress::getInstance()->debug()->_debug_add_content ( "File ( {$query} ).less generated successfully (rack)" );
				else
					Wordwebpress::getInstance()->debug()->_debug_add_content ( "File ( {$query} ).less reloaded successfully (rack)" );
				echo "<!--[{$key}]><link type=\"text/css\" rel=\"stylesheet\" href=\"" . WORDWEBPRESS_URL . Wordwebpress::getInstance()->configuration()->get_cfn_value ( 'assets_path' ) . Wordwebpress::getInstance()->configuration()->get_cfn_value ( 'cache_path' ) . '/_less_' . md5 ( $query ) . '.css" /><![endif]-->' . "\n";
			}
		}
		return $this;
	}
	 ...
	 * conforme as configura��es das pastas
	 *
	 * @param string $query
	 ...
	 * @return boolean
	 */
	private function generate_css_min($query) {
		$content = '';
		$root = WORDWEBPRESS_INCLUDE . Wordwebpress::getInstance()->configuration()->get_cfn_value ( 'assets_path' ) . '/css/';
		$files = explode ( ',', $query );
		if (sizeof ( $files ))
		...
		;
		if (! file_exists ( WORDWEBPRESS_INCLUDE . Wordwebpress::getInstance()->configuration()->get_cfn_value ( 'assets_path' ) . Wordwebpress::getInstance()->configuration()->get_cfn_value ( 'cache_path' ) . '/_css_' . md5 ( $query ) . '.css' )) :
			file_put_contents ( WORDWEBPRESS_INCLUDE . Wordwebpress::getInstance()->configuration()->get_cfn_value ( 'assets_path' ) . Wordwebpress::getInstance()->configuration()->get_cfn_value ( 'cache_path' ) . '/_css_' . md5 ( $query ) . '.css', $content );
		 elseif ($content != file_get_contents ( WORDWEBPRESS_INCLUDE . Wordwebpress::getInstance()->configuration()->get_cfn_value ( 'assets_path' ) . Wordwebpress::getInstance()->configuration()->get_cfn_value ( 'cache_path' ) . '/_css_' . md5 ( $query ) . '.css' )) :
			file_put_contents ( WORDWEBPRESS_INCLUDE . Wordwebpress::getInstance()->configuration()->get_cfn_value ( 'assets_path' ) . Wordwebpress::getInstance()->configuration()->get_cfn_value ( 'cache_path' ) . '/_css_' . md5 ( $query ) . '.css', $content );
		 else :
			return false;
		endif;
	...
	private function generate_less_min($query) {
		$content = '';
		$root = WORDWEBPRESS_INCLUDE . Wordwebpress::getInstance()->configuration()->get_cfn_value ( 'assets_path' ) . '/css/';
		$files = explode ( ',', $query );
		if (strpos ( $query, ',' ) === false) {
			$content .= file_get_contents ( $root . $query . '.less.css' );
		} else {
			foreach ( $files as $file )
				$content .= file_get_contents ( $root . $file . '.less.css' );
		}
		$lessc = new lessc ();
		...
		if (! file_exists ( WORDWEBPRESS_INCLUDE . Wordwebpress::getInstance()->configuration()->get_cfn_value ( 'assets_path' ) . Wordwebpress::getInstance()->configuration()->get_cfn_value ( 'cache_path' ) . '/_less_' . md5 ( $query ) . '.css' )) :
			file_put_contents ( WORDWEBPRESS_INCLUDE . Wordwebpress::getInstance()->configuration()->get_cfn_value ( 'assets_path' ) . Wordwebpress::getInstance()->configuration()->get_cfn_value ( 'cache_path' ) . '/_less_' . md5 ( $query ) . '.css', $content );
		elseif ($content != file_get_contents ( WORDWEBPRESS_INCLUDE . Wordwebpress::getInstance()->configuration()->get_cfn_value ( 'assets_path' ) . Wordwebpress::getInstance()->configuration()->get_cfn_value ( 'cache_path' ) . '/_less_' . md5 ( $query ) . '.css' )) :
			file_put_contents ( WORDWEBPRESS_INCLUDE . Wordwebpress::getInstance()->configuration()->get_cfn_value ( 'assets_path' ) . Wordwebpress::getInstance()->configuration()->get_cfn_value ( 'cache_path' ) . '/_less_' . md5 ( $query ) . '.css', $content );
		else :
			return false;
		endif;

...
class Wordwebpress_Javascript
{
	 ...
	 * por parametro seguindo a conven��o que todos os
	 * scripts estar�o na pasta %theme%/%assets_path%/javascripts
	 * caso n�o exista o script ele retorna um coment�rio
	 * ao html
	 *
	 * @param string $name
	 * @since 1.2
	 ...
	 */
	public function include_script_js_file($name, $rack = null, $reduce = true) {
		if (file_exists ( WORDWEBPRESS_INCLUDE . Wordwebpress::getInstance()->configuration()->get_cfn_value ( 'assets_path' ) . '/javascripts/' . $name . '.js' )) {
			if ($rack) :
				$this->_js ['rack'] [$rack] [$name] = $name;
				Wordwebpress::getInstance()->debug()->_debug_add_content ( "File: {$name}.js add on js container (rack)" );
			elseif (! $reduce) :
				$this->_js ['non_reduce'] [$name] = $name;
				Wordwebpress::getInstance()->debug()->_debug_add_content ( "File: {$name}.js add on js container (non reduce)" );
			else :
				$this->_js ['archy'] [$name] = $name;
				Wordwebpress::getInstance()->debug()->_debug_add_content ( "File: {$name}.js add on js container" );
			endif;
			return $this;
		}
		$this->_js ['error'] [$name] = $name;
		Wordwebpress::getInstance()->debug()->_debug_add_content ( "File: {$name}.js not existent", true );
		return $this;
	}
	/**
	 ...
	 * @return Wordwebpress_Javascript
	 */
	public function include_script_js($script) {
	 ...
	 * vari�vel para n�o ter perigo de instaciar
	 * duas vezes o mesmo script
	 *
	 * @since 1.2
	 * @return string|boolean
	 */
	public function get_swfobject_script() {
		if (! $this->swf_theme) {
			$this->swfObject = true;
			return $this->include_script_js ( 'vendor/swfobject' );
		}
		return false;
	}
	/**
	 * Imprime os arquivos e configura��es do js
	 * no tema
	 *
	 * @since 1.0
	 * @return Wordwebpress_Javascript
	...
	public function render_js() {
		if (Wordwebpress::getInstance()->configuration()->get_cfn_value ( 'jquery_online_version' ))
			echo "<script type=\"text/javascript\" src=\"https://ajax.googleapis.com/ajax/libs/jquery/" . Wordwebpress::getInstance()->configuration()->get_cfn_value ( 'jquery_version' ) . "/jquery.min.js\"></script>\n";
		else
			echo "<script type=\"text/javascript\" src=\"" . WORDWEBPRESS_URL . Wordwebpress::getInstance()->configuration()->get_cfn_value ( 'assets_path' ) . "/javascripts/vendor/jquery-" . Wordwebpress::getInstance()->configuration()->get_cfn_value ( 'jquery_version' ) . ".min.js\"></script>\n";
		
		$query = '';
		if (sizeof ( $this->_js ['non_reduce'] )) {
			sort ( $this->_js ['non_reduce'] );
			foreach ( $this->_js ['non_reduce'] as $key => $item ) {
				echo "<script type=\"text/javascript\" src=\"" . WORDWEBPRESS_URL . Wordwebpress::getInstance()->configuration()->get_cfn_value ( 'assets_path' ) . '/javascripts/' . $item . '.js"></script>' . "\n";
				Wordwebpress::getInstance()->debug()->_debug_add_content ( "File ( {$item} ).js added successfully (non reduce)" );
			}
		}
		...
		if (Wordwebpress::getInstance()->configuration()->get_cfn_value ( 'compress_css_js' )) {
			if (sizeof ( $this->_js ['archy'] )) {
				...
				if ($this->generate_js_min ( $query ))
					Wordwebpress::getInstance()->debug()->_debug_add_content ( "File ( {$query} ).js generated successfully" );
				else
					Wordwebpress::getInstance()->debug()->_debug_add_content ( "File ( {$query} ).js reloaded successfully" );
				
				echo "<script type=\"text/javascript\" src=\"" . WORDWEBPRESS_URL . Wordwebpress::getInstance()->configuration()->get_cfn_value ( 'assets_path' ) . Wordwebpress::getInstance()->configuration()->get_cfn_value ( 'cache_path' ) . '/_js_' . md5 ( $query ) . '.js"></script>' . "\n";
			}
			...
			if (sizeof ( $this->_js ['rack'] )) {
				foreach ( $this->_js ['rack'] as $key => $item ) {
					...
					echo "<!--[{$key}]><script type=\"text/javascript\" src=\"" . WORDWEBPRESS_URL . Wordwebpress::getInstance()->configuration()->get_cfn_value ( 'assets_path' ) . Wordwebpress::getInstance()->configuration()->get_cfn_value ( 'cache_path' ) . '/_js_' . md5 ( $query ) . '.js"></script><![endif]-->' . "\n";
				}
			}
		} else {
			...
			if (sizeof ( $this->_js ['rack'] )) {
				foreach ( $this->_js ['rack'] as $key => $item ) {
					...
					foreach ( $this->_js ['rack'] [$key] as $kkey => $iitem ) {
						Wordwebpress::getInstance()->debug()->_debug_add_content ( "File ( {$query} ).js generated successfully" );
						echo "<!--[{$key}]><script type=\"text/javascript\" src=\"" . WORDWEBPRESS_URL . Wordwebpress::getInstance()->configuration()->get_cfn_value ( 'assets_path' ) . '/javascripts/' . $iitem . '.js"></script><![endif]-->' . "\n";
					}
				}
			}
		}
		...
		if (sizeof ( $this->_js ['script'] )) {
			...
			foreach ( $this->_js ['script'] as $key => $item )
				echo $item;
			echo "\n</script>";
			Wordwebpress::getInstance()->debug()->_debug_add_content ( "Script added successfully on layout" );
		}
		
		if (sizeof ( $this->_js ['error'] )) {
			foreach ( $this->_js ['error'] as $key => $item )
				echo "\n<!-- script_js: " . WORDWEBPRESS_URL . Wordwebpress::getInstance()->configuration()->get_cfn_value ( 'assets_path' ) . '/javascripts/' . $item . '.js INEXISTENTE -->';
		}
		
		if ( $key = Wordwebpress::getInstance()->configuration()->get_cfn_value('keyAnalytics') )
		{
	 ...
	 * @return boolean
	 */
	private function generate_js_min($query) {
		$content = '';
		$root = WORDWEBPRESS_INCLUDE . Wordwebpress::getInstance()->configuration()->get_cfn_value ( 'assets_path' ) . '/javascripts/';
		$files = explode ( ',', $query );
		if (sizeof ( $files ))
		...
		if (! file_exists ( WORDWEBPRESS_INCLUDE . Wordwebpress::getInstance()->configuration()->get_cfn_value ( 'assets_path' ) . Wordwebpress::getInstance()->configuration()->get_cfn_value ( 'cache_path' ) . '/_js_' . md5 ( $query ) . '.js' )) :
		file_put_contents ( WORDWEBPRESS_INCLUDE . Wordwebpress::getInstance()->configuration()->get_cfn_value ( 'assets_path' ) . Wordwebpress::getInstance()->configuration()->get_cfn_value ( 'cache_path' ) . '/_js_' . md5 ( $query ) . '.js', $content );
		elseif ($content != file_get_contents ( WORDWEBPRESS_INCLUDE . Wordwebpress::getInstance()->configuration()->get_cfn_value ( 'assets_path' ) . Wordwebpress::getInstance()->configuration()->get_cfn_value ( 'cache_path' ) . '/_js_' . md5 ( $query ) . '.js' )) :
		file_put_contents ( WORDWEBPRESS_INCLUDE . Wordwebpress::getInstance()->configuration()->get_cfn_value ( 'assets_path' ) . Wordwebpress::getInstance()->configuration()->get_cfn_value ( 'cache_path' ) . '/_js_' . md5 ( $query ) . '.js', $content );
		else :
		return false;
		endif;
